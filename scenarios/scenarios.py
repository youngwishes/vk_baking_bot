...
class Scenario(SingleToneMixin):
    """
    По сути это связный список, который реализует конкретный сценарий.
    Сценарий - последовательность функций, куда попадает сообщение пользователя.
    Таким образом реализована машина состояний. Чтобы создать новый сценарий необходимо отнаследоваться
    от этого класса.
    """
    def __init__(self):
        self.tail = None
